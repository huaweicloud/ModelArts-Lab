...
TRAIN_DATASET = 'winequality-red.csv'
...
LOCAL_DATA = '/tmp/winequality-red.csv'
...
spark = pyspark.sql.SparkSession.builder.config("spark.driver.host", "localhost").master("local[*]").appName(
    "wine_quality-regression").getOrCreate()
metric_dict = {}
...
def download_dataset():
    ...
    try:
        ...
        train_file = data_path.split("/", 1)[1] + "/winequality-red.csv"
        print("train_file")
...
def create_config():
    ...
    :return:
    """
    schema_model = json.loads('{"model_algorithm":"wine_classification","model_source":"custom","tunable":"false","model_type":"Spark_MLlib","metrics":{},"apis":[{"protocol":"http","url":"/","method":"post","request":{"Content-type":"application/json","data":{"type":"object","properties":{"data":{"type":"object","properties":{"req_data":{"type":"array","items":[{"type":"object","properties":{"fixed acidity":{"type":"number"},"volatile acidity":{"type":"number"},"citric acid":{"type":"number"},"residual sugar":{"type":"number"},"chlorides":{"type":"number"},"free sulfur dioxide":{"type":"number"},"total sulfur dioxide":{"type":"number"},"density":{"type":"number"},"pH":{"type":"number"},"sulphates":{"type":"number"},"alcoho":{"type":"number"}}}]}}}}}},"response":{"Content-type":"application/json","data":{"type":"object","properties":{"resp_data":{"type":"array","items":[{"type":"object","properties":{"predictionClass":{"type":"string"}}}]}}}}}]}',object_pairs_hook=OrderedDict)

    res_properties = collections.OrderedDict()
    col_num = 10
    input_name_list = ["fixed acidity", "volatile acidity","citric acid","residual sugar",
                                                             "chlorides","free sulfur dioxide","total sulfur dioxide","density",
                                                             "pH","sulphates",'alcoho']
    input_type_list = ["number", "number", "number", "number","number", "number", "number", "number", "number", "number"]
