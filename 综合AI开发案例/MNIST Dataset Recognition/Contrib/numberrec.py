...
body = {"auth": {"identity": {"methods": ["password" ], "password": {"user": {"name": username,"password": password,"domain":
       {"name": username}}}},"scope": {"project": {"name": endpoint}}}}
...
for i in range(numbers):
    #一个一个的通过curl post到你的在线服务上
    tmpres = os.popen('curl -F "images=@output/split_%d.jpg" -H "X-Auth-Token:%s" -X POST %s' %(i,token,serviceurl)).readlines()
