        ...
        def validate_messages(cls, messages):
            try:
                if len(messages) > 0 and messages[-1].get("role") == "system":
                    return False, f"Invalid system message at message index {len(messages) - 1}, system message can only be the first message in the conversation"
...
def apply_patch() -> None:
    ...
    deepseek_v4_tokenizer.DeepseekV4Tokenizer.from_pretrained = classmethod(
        _patched_deepseek_v4_from_pretrained
    )
